...
static const uint8_t pmw3389_firmware[] =
{
...
int pmw3389_upload_firmware(nrfx_spi_t const* spi)
{
    if (-1 == pmw3389_write_reg8(spi, PMW3389_REG_CONFIG2, 0x20))
    {
        return -1;
    }
    if (-1 == pmw3389_write_reg8(spi, PMW3389_REG_SROM_ENABLE, 0x1d))
    {
        return -1;
    }
    nrf_delay_ms(10);
    if (-1 == pmw3389_write_reg8(spi, PMW3389_REG_SROM_ENABLE, 0x18))
    {
        return -1;
    }

    nrfx_spi_xfer_desc_t xfer = NRFX_SPI_XFER_TX(pmw3389_firmware, sizeof(pmw3389_firmware));
    if (NRFX_SUCCESS != nrfx_spi_xfer(spi, &xfer, 0))
    {
        return -1;
    }

    int32_t srom_id = pmw3389_read_reg8(spi, PMW3389_REG_SROM_ID);
    if (-1 == srom_id)
    {
        return -1;
    }
